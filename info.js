...
function setActiveButton(btn) {
  [penBtn, brushBtn].forEach((b) => b.classList.remove("active"));
...
function togglePalette(show) {
  ...
  palette.setAttribute("aria-hidden", !show);
}
colorPocket.addEventListener("click", () => {
  togglePalette(palette.style.display !== "block");
});
...
document.addEventListener("click", (e) => {
  if (!colorPocket.contains(e.target) && !palette.contains(e.target))
    togglePalette(false);
});
...
function draw(e) {
  ...
  e.preventDefault();
  const p = getXY(e);
  strokeLine(last.x, last.y, p.x, p.y);
  last = p;
}
...
function strokeLine(x1, y1, x2, y2) {
  ...
  if (tool === "brush") {
    ...
    for (let i = 0; i < 2; i++) {
      ctx.beginPath();
      ctx.moveTo(
        x1 + (Math.random() - 0.5) * 1.4,
        y1 + (Math.random() - 0.5) * 1.4
      );
      ctx.lineTo(
        x2 + (Math.random() - 0.5) * 1.4,
        y2 + (Math.random() - 0.5) * 1.4
      );
      ctx.stroke();
    }
  } else {
